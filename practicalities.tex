...
shell to get status information.  Several built in functions make use
of this, and the \texttt{status} module makes it possible to insert
status reporting into any method.

The built in status will report the processing state, if it is in
\prepare, \analysis, or \synthesis.  Iterators report which dataset
(perhaps in a chain) that is currently being iterated, and the
\texttt{blob} functions report status of file pickling.

Here is an example of how to use the status module
\begin{python}
