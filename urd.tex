...
\starttabletwo
\RPtwo timestamp & Timestamp of session\\[1ex]
\RPtwo caption & A caption\\[1ex]
\RPtwo user/automata & Location of urd-list\\[1ex]
\RPtwo joblist & An object of type \texttt{joblist}, containing all jobs built in the session.  For more information, see~\ref{xx}.\\[1ex]
\RPtwo deps & A dictionary of dependencies from \texttt{user/automata} to urd sessions, like this \mintinline{python}@{user/automata: session}@.\\[1ex]
\stoptabletwo
...
before a certain date in time should be discarded, do like this
\begin{python}
urd.truncate('test', '20160930')
\end{python}
This will rollback everything that has happened in the \texttt{test}
list back to '20160930'.  There is also a special case,
\begin{python}
urd.truncate('ab/test', 0)
\end{python}
that will reset t

Remember, internally Urd stores the complete
