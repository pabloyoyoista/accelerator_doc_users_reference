...
character means itself.  This supports standard CSV escaping,
i.e. doubling the quote character. \texttt{'a''b'} $\rightarrow$ \texttt{a'b}

Unquoting applies to any field that starts with a quote character, quote
characters inside other fields are not treated specially.\\

\RP \texttt{labelsonfirstline} & \pyTrue & If set to
\pyTrue, data on the first line of the file will
be used as column labels.  These will be decoded with
\py{str.decode('utf-8', 'backslashreplace')}. If \pyFalse, labels must
...
If specified, these labels always apply, regardless of
...
Renaming happens before discard.  It accepts a dictionary
...
in the dataset \texttt{skipped}.  All columns (except the \texttt{lineno}
column) will be of type \py{bytes}.  Typically, the dataset from
...
Each line is limited to slighly less than 16MB.  However, this is just a
constant in the code (\texttt{BIG\_Z} in \texttt{standard\_methods/csvimport.py})
that is by default set to a value that allows the Accelerator to run on low
memory platforms (it will allocate $sliceno * 2 + 3$ buffers of this size).
If you have enough memory you can set it as high as $2^{30} - 1 = 1073741823$
(1GB), but higher than that will not work correctly.
...
\RP \texttt{inside\_filenames} & \texttt{\{\}}&
   ...
   name, \py{{'filename in zip': 'dataset name', ...}}.
...
\RP \texttt{chaining}          & \texttt{on}  &
  Can be one of:
  \begin{itemize}
  \item[]\texttt{off} -- Do not chain the imports.  Use \py{datasets.previous} on all datasets.
  \item[]\texttt{on} -- This is the default, and means \texttt{by\_dict} if \texttt{inside\_filenames}
  is set and \texttt{by\_ziporder} if it is not.
  \item[]\texttt{by\_filename} -- Chain the imports in filename order.
  \item[]\texttt{by\_dsname} -- Chain the imports in dataset name order.
  \item[]\texttt{by\_dict} -- Chain the imports in the order in \texttt{inside\_filenames}.
  Note that you must pass \texttt{inside\_filenames} an \py{OrderedDict} for this.
  \item[]\texttt{by\_ziporder} -- Chain the imports in the order the files are in the zip file.
\end{itemize}\\
...
\RPnotitle  \texttt{number:int} && \texttt{int}, but accepts float-formatted ints.\\
\stoptablenotitle
\noindent Floats have standard double precision (64 bits), ints can be up to
$\pm (2^{1007}-1)$.
...
but have trailing zero decimals. It is especially useful if the numbers
are too big to be represented exactly as doubles (i.e. more than $2^{53}$).


\subsubsection{Floating Point Numbers}
Floating point numbers may be stored as 32 or 64 bits.
