...
\RPtwo \texttt{py2} (\texttt{py27}, \texttt{py3}, $\dots$) & Path to
Python executable.  There could be any number of Python executables
listed using keys starting with ``\texttt{py}'' followed by a set of
numbers, such as \texttt{py2}, \texttt{py3}, \texttt{py35} etc.  The
file \texttt{methods.conf} (see section~\ref{sec:methods_conf}) is
used to map a method to a specific executable.
