...
executed once without parallellisation.  In order to get the method to
execute, it is called from a \textsl{build script} looking something
like this
\begin{python}
def main(urd):
    ...
    print(job.load())
\end{python}
The \texttt{main()}-function is called when the build script is run.
The input parameter \texttt{urd} is an object that contains functions
for job building and organisation, and is described in
...
\clearpage
\noindent The most important files in a job directory are
\begin{itemize}
...
data stored in this directory.  For example, the job's return value
can be loaded into a variable using the \texttt{.load()} function, and
the metadata is pretty printed using the \texttt{ax job} shell
command.
...
A workdir is where jobs arae stored.  There can be more than one
...
This further processing is represented by simply printing the result
to standard output.  A new method \texttt{print\_result} is created
for this task, and it goes like this
\begin{python}
...
\end{python}
The \texttt{print\_result} method loads the result from the provided
job and prints its contents to \texttt{stdout}.  Note that this method
...
each reading intermediate results from previous ones, without the need
for explicit filenames.  The Accelerator will keep track of all job
dependencies, so there is no doubt which jobs that are run when and on
which data.  And since the Accelerator remembers if a job has been
executed before, it will link and re-use previous jobs when possible,
thereby saving time.  Also note that a re-used job is proving that the
code, input- and output data is unchanged and connected.

% Parallel Processing intro goes here!
