...
running job.  See section~\ref{sec:classes:job} for details about these classes.
...
``\mintinline{shell}{method packages}'', see section~\ref{sec:configfile}.
...
server is using.  The Accelerator and its \texttt{standard\_methods}
...
The Accelerator server will suggest adding modules to a source file in
...
accessing sliced datasets, see for example
chapter~\ref{chap:iterators} for its use in dataset iterators.
...
the Accelerator server was started \textsl{and} to a file in the
...
(Datasets will be covered in chapter~\ref{chap:datasets}).  The \texttt{CurrentJob}
...
useful to a method during execution.  \textsl{This section just
  ...
  See section~\ref{sec:classes:job} for a complete list of the functionality.}
...
\section{Converting Between Jobs and Datasets}
...
\end{python}
or using the \texttt{Dataset} constructor.
\begin{python}
