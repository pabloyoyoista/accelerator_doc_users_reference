...
\comment{BEGIN old stuff}
\subsection*{method and work directories}
Consider the following figure:
\begin{figure}[h!]
  \begin{center}
    \input{figures/method2.pdftex_t}
    \label{fig:method}
  \end{center}
\end{figure}


\noindent The figure shows a method and a job that was built from that
particular method.  For isolation purposes, methods are stored
in \textsl{method directories}, and jobs (job directories) are stored
in \textsl{work directories}.  There could be any number of method and
work directories, but they all have to be specified explicitly in the
Accelerator's configuration file in order to be taken into account.
