...
\subsection{number}
number: integer when possible (up to +-2**1007-1), float otherwise
number:int Never float, but accepts int.0 (or int.00 and so on)
64 bytes + 1 byte marker, i.e. min 9 bytes.
