...
\begin{python}
# return the chain object from the 'default' dataset in jobid
chain = Dataset(jobid).chain()
\end{python}
...
\subsection{Bitmasks}
The bitmask types are stored as 32 or 64 bits of data in a dataset,
and is represented by unisgned integers in the Python code.
...
\texttt{firstset}.  If the name is omitted, the name
...
A hashed dataset is created by setting the \texttt{hashlabel} argument
when calling \texttt{DatasetWriter}.  Note that for a hashed dataset,
only data fulfilling the hashing requirement for a slice may be
written to that slice.  A straightforward write will cause an
exception if the data does not belong to the slice.
...
that belongs to another slice are silently ignored, while ``correct''
data gets written as expected.
...
\end{python}
This is also beneficial if it is expensive to compute the data to be
...
documentation in \texttt{dataset.py}.
...
def synthesis(params):
    ...
    for sliceno in range(params.slices):
        ...
        for data in datasets.source.iterate(sliceno, ...):
            ...
            dw.write(x)
\end{python}
Note the \texttt{for}-loop over \texttt{sliceno}, which controls both
the reading iterator \textsl{and} the dataset writer.
