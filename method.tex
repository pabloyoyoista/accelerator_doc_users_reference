...
The method name can be followed by one or more whitespaces and a token
such as \texttt{py}, \texttt{py2} or \texttt{py35}, indicating the
Python version to use when executing the method.  Python executables
are defined in the configuration file, see
section~\ref{sec:configfile}.  If this field is left empty, the
default executable \texttt{py} is selected, which always corresponds
to the same executable as the currently runing Accelerator daemon is
using.



The field can be left blank if the
method works on any Python version.  Here is an example
\begin{shell}
# this is a comment
...
where the first two runs on Python2 and Python3 respectively (assuming
a sensible binding in the Accerator's configuration file).  The method
\texttt{testx} will use the \texttt{py} executable, which points to
the executable used to run the current Accelerator daemon processes.
Finally, the method \texttt{bogusmethod} is commented out and trying
to build it will issue an error.
...
available to the runner.  A workaround is to link the dataset to the
