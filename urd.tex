...
and keep track of jobs.  Urd comes into play when simple build
scripting is not enough.  Build scripts are capable of a large variety
...
fully transparent and under control.

Using Urd, a project can be separated into functionally independent
parts, or \textsl{sessions}, and all dependencies between jobs inside
as well as between these sessions are tracked by a \textsl{transaction
log}.  Even though there is no need to, since all previous results are
stored, any part or session of a project can be re-constructed the way
it looked at any instance in time.

More formally, Urd provides two things:
\begin{itemize}
\item[1.] Separation between build scripts,
          and a way to share information about existing jobs between
          different scripts (or the same script at different points in
          time).
\item[2.] A searchable transaction log database of all jobs,
          together with their dependencies on other jobs.  A
...
%@@@ kanske kortare och i introt??

A simple use case is used here to illustrate the basic Urd features.

Assume a project where movie recommendation data is to be analysed.
Every hour, new data corresponding to the last hour's activity is
added to the project in the shape of log files.  The project is based
on two build scripts, an \textsl{import} script, and
an \textsl{analysis} build script.
...
jobs} that have been built based on that file.  Typically, this
includes \texttt{csvimport}, \texttt{dataset\_type}, perhaps
a \texttt{dataset\_hash\_partition}, and similar data pre-processing
jobs.
...
``message passing'' between build scripts.  The import scripts could
be modified or even removed, but as long as no workdirs are cleared
and the Urd database is intact, it is still possible to fetch
references to the existing import jobs from Urd.  Thus, it is always
possible to go back and see what things looked like at any previous
point in time.
...
happen that there is any uncertainty about which data that was used
for a particular analysis.

To investigare the result of an analysis, Urd can be asked to return
...
As pointed out in the previous secrtion, dependencies will be recorded
...
\RPtwo \texttt{method} & Name of method to build.  Enter \texttt{test}
    ...
    Acclerator will look for this file in all method directories
...
Flags can be added to a build script ut run time using a comma
