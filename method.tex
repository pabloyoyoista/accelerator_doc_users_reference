...
followed by one or more whitespaces and a token such as
``\texttt{py2}'' or ``\texttt{py3}'', indicating the Python version to
use when executing the method.  Python executables are defined in
the configuration file, see section~\ref{sec:configfile}.  The field
can be left blank if the method works on any Python version.  Here is
an example
\begin{shell}
# this is a comment
