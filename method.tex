...
functions that are useful to a method during execution.  \textsl{This
  section just provides a taste of the most common operations that are
...
data and datasets to disk.  (Dataset writing is the topic of
section~\ref{}.)  Files may be written using the \texttt{job.save()}
and \texttt{job.json\_save()} functions.  The first writes a Python
``pickle'' file, and the latter uses \texttt{json} encoding.  For
example
\begin{python}
def synthesis(job):
...
parameters \options, \datasets, \jobids, and \texttt{caption} are
...
\texttt{override\_previous} option, which takes a jobid (or \pyNone)
