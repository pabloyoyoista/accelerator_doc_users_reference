...
\RPtwo   \texttt{float64}   &  64 bit float (this is what the Python \py{float} type uses)\\
...
\RPtwo   \texttt{date}      &  \py{datetime.date}\\
\RPtwo   \texttt{time}      &  \py{datetime.time}\\
\RPtwo   \texttt{datetime}  &  \py{datetime.datetime}\\[1ex]

\RPtwo   \texttt{bytes}     &  raw data \\
\RPtwo   \texttt{ascii}     &  string limited to ascii \\
\RPtwo   \texttt{unicode}   &  normal string \\[1ex]

\RPtwo   \texttt{json}      &  anything jsonable (anything \py{json.dump} accepts) \\
\RPtwo   \texttt{pickle}    &  anything picklable (anything \py{pickle.dump} accepts) \\
\stoptabletwo
...
\texttt{float64}, but also lower precision. The python \py{float} type
has the same precision as \texttt{float64}, so that is the safe choice.
If you use \texttt{float32} you may be surprised by e.g. \py{0.1} not
...
corresponding classes from the standard \py{datetime} module. A column that is
...
with the \py{'surrogateescape'} error handler).
...
On Python2, the \texttt{ascii} type will return \py{str} objects
(a.k.a. \py{bytes}) and \texttt{unicode} will return \pytwo{unicode} objects.

On Python3, both return \py{str} objects (i.e. what Python2 called
\pytwo{unicode}.)
