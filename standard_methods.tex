...
create a soring indexing column that stipulates the sorting order.
Each column is then read, one at a time, and written according to the
sorting column.  All columns except the sorting columns are read as
raw data, meaning that no de-serialising into Python's internal format
is required.  This makes sorting fast and relatively low on memory
useage.  However, since it has to hold the index as well as one full
column of data in memory, the method has in theory limited sorting
capability.  Still, a standard computer can sort very large datasets
without trouble.
