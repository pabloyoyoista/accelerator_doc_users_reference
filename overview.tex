...
The following example method is very simple.  It does not use any
features and it does not take any input parameters, it will just
return the string ``\texttt{hello world}'' and exit.
\begin{python}
def synthesis():
    return "hello world"
\end{python}
...
When execution is completed, a single object of the class \texttt{Job}
is the only thing that is returned to the user.  In the example above,
the variable \texttt{job} will be of type \texttt{Job}.  This object
provides a convenient interface to the data in the corresponding job
directory.
...
recalling!  In order to have the job executing again, we have to
change either the source code or input parameters.
...
The \texttt{Job} class provides conveniently access to the important
